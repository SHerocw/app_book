...
function Obj(el){
	this.el = el; // 2
	if(typeof el != 'undefined')this.length = el.length;
}
Obj.prototype = {
	...
	animate : function(obj,time,fn,type){ // 动画
		...
		for(let k = 0;k < ele.length;k ++){
			ele[k].start = {}; // 用对象储存可以存入多个值，且属性名与obj相同，可以在obj的循环中使用它的变量
			for(var i in obj){
				ele[k].start[i] = i == 'scrollTop' ? ele[k].scrollTop : parseFloat(getStyle(ele[k],i));
			}
			clearInterval(ele[k].time); // 给不同标签添加定时器，清除对应标签的定时器
			ele[k].time = setInterval(function(){
				t ++;
				var flag = true;
				for(var i in obj){
					if(t >= parseInt(d)){
